...
tcGmmPen <- function(y, g, zInit, maxIter = 100, thresh = 1e-16, lambda = 1, verboseN = TRUE) {
  ...
  for(k in 1:maxIter) {
    if(k == 1) {
      ...
      dat_list <- lapply(1:n.group, function(j){
        dat_all[z[, j] == 1, ]
      })
      ...
      fit_list <- lapply(1:n.feature, function(i) {
        lapply(1:n.group, function(j){
          dat <- dat_all
          fit <- glmnet::glmnet(y = dat[, i], x = cbind(dat[, n.feature + 1], 0), family = "gaussian", weights = dat[, n.feature + 1 + j], alpha = 1, lambda = lambda)
          fit
        })
      })
    }
    ...
    if(k >= 2 &&  norm(mu_curr - mu_old, "F") < thresh) {cat(paste0("Iteration ends in ", k, "\n")); break}
  }
